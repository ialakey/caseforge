...
pnpm test:smoke:security  # заголовки, лимиты, инъекции, роли
...
`test:smoke:security` проверяет то, что легче всего снести по неосторожности:
что заголовки безопасности по-прежнему стоят на каждом ответе, что payload'ы с
инъекциями уходят в базу значением, а не синтаксисом, что токен с ролью, которой
у аккаунта нет, отбивается, что лимит на размер тела держится везде, кроме
единственного маршрута загрузки документов, и что потолок по запросам
срабатывает, а health-check при этом остаётся доступным. Что именно каждая из
этих проверок защищает — в разделе 13 [ARCHITECTURE.ru.md](docs/ARCHITECTURE.ru.md).
...
| `TRUST_PROXY` | `true` только когда соединения терминирует обратный прокси и `X-Forwarded-For` ставит он сам. Если API открыт напрямую, доверие к этому заголовку позволяет каждому выбрать себе адрес, а вместе с ним и свою корзину лимита |
| `RATE_LIMIT_MAX`, `RATE_LIMIT_WINDOW_SEC` | общий потолок по запросам на адрес. По умолчанию 300 в минуту — одна загрузка страницы до него не достаёт |
...
лицензией не покрыт. Спрашивайте: <alakov.ilia@gmail.com>.
