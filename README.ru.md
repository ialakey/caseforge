...
- инвентарь сайта: фильтры по состоянию и ценовому диапазону, продажа одного
  предмета или сразу всего на экране с подтверждением, заглушка вывода
...
предметами, кейс-батлы, промокоды, KYC.
...
**Вывод из инвентаря — заглушка.** Он помечает предмет выведенным и больше не
делает ничего: ни бота, ни трейд-оффера, ни очереди. Настоящий путь — модуль
выводов, описанный ниже, и заглушка его не трогает.
...
продажи, рейт-лимит открытий, кулдаун колеса и сами сектора колеса лежат в базе и
читаются в момент запроса, поэтому изменение — это сохранение, а не деплой.
...
apps/
  api/
    ...
    src/inventory/           инвентарь: фильтры, продажа, заглушка вывода
